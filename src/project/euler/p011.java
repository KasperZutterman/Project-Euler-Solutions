...
public class p011 {
    ...
    public String run() {
        ...
        for (int y = 0 ; y < SQUARE.length; y++) {
            for (int x = 0; x < SQUARE[y].length; x++) {
                maxProd = Math.max(product(x, y, 1,  0, 4), maxProd);
                maxProd = Math.max(product(x, y, 0,  1, 4), maxProd);
                maxProd = Math.max(product(x, y, 1,  1, 4), maxProd);
                maxProd = Math.max(product(x, y, 1, -1, 4), maxProd);
            }
        }
    ...
    private static int product (int x, int y, int dx, int dy, int n) {
        if(!isInSquare(x + (n - 1) * dx, y + (n - 1) * dy)) {
            return -1;
        }
        else {
            int prod = 1;
    ...
    private static boolean isInSquare(int x, int y) {
        return (0 <= y) && (y < SQUARE.length) && (0 <= 0) && (x < SQUARE[y].length);
    }
